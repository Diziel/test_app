...
export const tableSlice = createSlice({
  ...
  reducers: {
    ...
    deleteRow: (state, action: PayloadAction<{ id: number; chosenData: string }>) => {
      const { chosenData } = action.payload;
      state[chosenData] = state[chosenData].filter((row) => row.id !== action.payload.id);
    },
    editRow: (state, action: PayloadAction<{ id: number; newData: RowData; chosenData: string }>) => {
      const { id, newData, chosenData } = action.payload;
      const index = state[chosenData].findIndex((row: RowData) => row.id === id);
      if (index !== -1) {
...
export const { addRow, deleteRow, editRow, addClonedData, setIsCloneVisible } = tableSlice.actions;

export const selectTableData = (state: RootState) => state.table.data;
export const selectClonedTableData = (state: RootState) => state.table.clonedData;
